...
RUN rabbitmqctl add_user flyersconcierge ZXCflight
RUN rabbitmqctl set_user_tags flyersconcierge administrator
RUN rabbitmqctl set_permissions -p / flyersconcierge ".*" ".*" ".*"
RUN RUN virtualenv venv
RUN mkdir /app
ADD . /app/
WORKDIR /app
